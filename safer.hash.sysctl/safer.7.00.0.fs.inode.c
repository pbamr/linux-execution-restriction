 ...
 */
	...
	/* Nur wenn die Datei das Execute-Bit gesetzt hat */
	//if ((inode->i_mode & S_IXUGO))
	//	inode->i_boettcher_flags = 0;

	clear_bit(0, (unsigned long *)&inode->i_boettcher_flags);
	clear_bit(1, (unsigned long *)&inode->i_boettcher_flags);


	//if (S_ISREG(inode->i_mode) && (inode->i_flags & S_PRIVATE)) {
