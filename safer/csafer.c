...
s64 Add(void *self, char *newstring)
{
	...
	if (struct_tstringlist->TStringList_Lines == -1) {
		struct_tstringlist->TStringList = calloc(1, sizeof(char *));
		if (struct_tstringlist->TStringList == NULL) return(-1);
		
		struct_tstringlist->TStringList[0] = calloc(strlen(newstring) + 1, sizeof(char));
	...
	//MEM Columns
	struct_tstringlist->TStringList[struct_tstringlist->TStringList_Lines] = calloc(strlen(newstring) + 1, sizeof(char));
	if (struct_tstringlist->TStringList[struct_tstringlist->TStringList_Lines] == NULL) {
		//former state
...
s64 DynStrcat (void *self, s64 number, char *newstring)
{
	...
	s64 len = strlen(struct_tstringlist->TStringList[number]) +  strlen(newstring);
...
s64 DelDup(void *self)
{
	...
	for (s64 n = 0; n < struct_tstringlist->TStringList_Lines - 1; n++) {
		for (s64 n0 = n + 1; n0 < struct_tstringlist->TStringList_Lines - 1; n0++) {
			if (strcmp(struct_tstringlist->TStringList[n], "")  == 0) break;
			if (strcmp(struct_tstringlist->TStringList[n], struct_tstringlist->TStringList[n0]) == 0) {
				strcpy(struct_tstringlist->TStringList[n0], "");
			}
		}
	}
	...
	for (s64 n = struct_tstringlist->TStringList_Lines - 1; n > 0; n--) {
		if (strcmp(struct_tstringlist->TStringList[n], "") == 0) {
			if (n == struct_tstringlist->TStringList_Lines - 1) {
				free(struct_tstringlist->TStringList[--struct_tstringlist->TStringList_Lines]);
				struct_tstringlist->TStringList[struct_tstringlist->TStringList_Lines] = NULL;
			}
			else {
				strcpy(struct_tstringlist->TStringList[n], struct_tstringlist->TStringList[--struct_tstringlist->TStringList_Lines]);
				free(struct_tstringlist->TStringList[struct_tstringlist->TStringList_Lines]);
				struct_tstringlist->TStringList[struct_tstringlist->TStringList_Lines] = NULL;
			}
		}
...
s64 LoadFromFile(void *self, char *file_name)
{
	...
	for (int n = 0; n < max_bytes; n++) {
		if (TEXT[n] == '\n') {
			if (n > start) {
				//struct_tstringlist->TStringList[lines] = (char *) malloc((n - start + 1) * sizeof(char));
				//strncpy(struct_tstringlist->TStringList[lines], &TEXT[start], n - start);
				//ERROR CHECK
				struct_tstringlist->TStringList[lines] =  calloc (struct_tstringlist->TStringList_Length_Max + 1, sizeof(char));
				strncpy(struct_tstringlist->TStringList[lines], &TEXT[start], n - start);
...
int ErrorMessage()
{
	printf("fpsafer, 2022/05 Peter Boettcher, Germany, Muelheim Ruhr\n");
