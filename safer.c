		...
		const char __user *const __user *, list)
{
		...
		switch(sub_number) {
			...
			case 7:		if (list == NULL) {		/* check? */
					...
					*/
					...
					if (allow_list_max < 1) {
#ifdef PRINTK
						printk("NO ALLOW USERLAND TABELLE\n");
#endif
						return(-1); 
					}
			...
			case 8:		if (list == NULL) {				/* check? */
					...
					if (deny_list_max < 1) { 
#ifdef PRINTK
						printk("NO DENY USERLAND TABELLE\n"); 
#endif
						return(-1); 
					}
	...
	if (safer == 0) {
		/* root allowed */
		switch(user_id) {
			case 0:		if (strncmp("/bin/", filename, 5) == 0) break;
			...
			default:
					...
					/* Importend! Need qsorted list */
					if (besearch(str_file_name, allow_list, allow_list_max) == 0) goto prog_allowed;
